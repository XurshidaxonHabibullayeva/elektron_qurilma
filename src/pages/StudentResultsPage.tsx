  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        description="Har bir dars bo‘yicha topshirgan testlaringiz balli (oxirgi urinish saqlanadi)."
      />
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <StatCard
          ...
          value={
            loading ? '…' : new Set(rows.map((r) => r.lesson_id).filter(Boolean)).size
          }
      ...
      <Card className="overflow-hidden p-0">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-700/80">
          <h2 className="text-base font-semibold text-slate-900 dark:text-white">Test tarixi</h2>
          <Button
            type="button"
            variant="ghost"
            onClick={() => {
          ...
          <table className="w-full min-w-[400px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700/80">
              {loading ? (
                ...
                <tr>
                  <td colSpan={3} className="px-5 py-10 text-center text-slate-500 dark:text-slate-400">
                    Hozircha natija yo‘q. Dars sahifasidan test topshirgach, balllaringiz shu yerda
                    ko‘rinadi.
                  </td>
                </tr>
              ) : (
