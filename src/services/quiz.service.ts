    ...
    return {
      ...
      lesson: r.lesson
        ? {
            ...
            class: r.lesson.class
              ? { name: String(r.lesson.class.name) }
              : null,
            subject: r.lesson.subject
              ? { name: String(r.lesson.subject.name) }
              : null,
          }
        : null,
      student: r.student
        ? {
            full_name: r.student.full_name ? String(r.student.full_name) : null,
          }
        : null,
    };
  });
...
export async function fetchMyResultsForLesson(
  lessonId: string,
): Promise<QuizResultRow[]> {
  const { data, error } = await supabase
    .from("results")
    .select(
      "id, student_id, lesson_id, score, total_questions, answers, attempt_number, created_at",
    )
    .eq("lesson_id", lessonId)
